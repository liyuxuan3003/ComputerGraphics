...
基样条曲线的想法类似于\xref{fml:基数三次曲线构成的分段曲线的混合函数}，我们曾研究过基数三次曲线构成的分段曲线作为一个整体的混合函数，这样的混合函数本身也是一个三次的分段函数。这就提示我们，假如能找到一种方法，能对于任意$d$次，产生一个恰当的$d$次的分段函数作为混合函数，这个混合函数就应当是某个$d$次单一曲线构成的分段曲线的混合函数，且不需要了解这个单一曲线的性质。
...
\xref{fig:基样条曲线的混合函数}展示了$n=7$个控制点下$d=3$次基样条曲线的混合函数。和\xref{fig:基数三次曲线构成的分段曲线的混合函数}一致，这种情况下会有$n+d+1$个节点，但只有中央$n-d+1$个节点是有效范围，首末$d$个节点在定义域外。
