...
\begin{BoxFormula}[Metaballs衰减滤波函数]
    ...
    \begin{Equation}
        g(d)=\begin{cases}
            1-3\qty(\dfrac{d}{r})^2,&0\leq d\leq r/3\\[8mm]
...
\begin{Figure}[衰减滤波函数]
    \includegraphics[scale=0.8]{Chapter16A_01a.fig.pdf}
\end{Figure}
...
我们在\cref{tab:两个逐渐靠近的点构成的曲面}中展示了两个逐渐靠近的点产生的曲面，取$r=1$，采用Soft Objects衰减滤波函数，令$\te{iso}=0.2$。观察到随着距离靠近，原本独立的两个球面逐渐相互吸引并融合在一起。
\begin{Table}[两个逐渐靠近的点构成的曲面]&!!
    \begin{tblr}
    {
        ...
        \includegraphics[scale=0.68]{Chapter16A_02a.fig.pdf}&
        \includegraphics[scale=0.68]{Chapter16A_02b.fig.pdf}&
        \includegraphics[scale=0.68]{Chapter16A_02c.fig.pdf}\\
        $(\pm 0.4,0,0)$&$(\pm 0.2,0,0)$&$(\pm 0.0,0,0)$\\
        \includegraphics[scale=0.68]{Chapter16A_02d.fig.pdf}&
        \includegraphics[scale=0.68]{Chapter16A_02e.fig.pdf}&
        \includegraphics[scale=0.68]{Chapter16A_02f.fig.pdf}\\
    \end{tblr}
\end{Table}